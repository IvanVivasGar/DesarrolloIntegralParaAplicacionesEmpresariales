...
def modify_category(name: str = Query, newName: str = Query):
    ...
    for item in categories:
        if item["name"].lower().strip().replace(" ", "") == name.lower().strip().replace(" ", ""):
            item["name"] = newName
            for book in books:
                if book["category"].lower().strip().replace(" ", "") == name.lower().strip().replace(" ", ""):
                    book["category"] = newName
                updated_books.append(book)
            return JSONResponse(status_code=200, content={"message": "La categoría se modificó correctamente.", "updated_books": updated_books})
